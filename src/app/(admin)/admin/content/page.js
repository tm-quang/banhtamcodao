    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4 mb-2">
        <div className="flex flex-col">
          <div className="flex items-center gap-2.5 mb-0.5">
            ...
            <h1 className="text-2xl font-black text-gray-900 tracking-tight mb-2">Quản lý nội dung</h1>
          </div>
          <p className="text-[11px] font-bold text-gray-600 uppercase tracking-[0.15em] ml-0.5">Quản lý đánh giá khách hàng & Testimonials ({testimonials.length} nội dung)</p>
        </div>
          ...
          <>
            <div
              ...
              <div className="relative z-10 flex flex-col h-full justify-between">
                ...
                <div>
                  <p className="text-xl md:text-3xl font-black mb-0.5 tabular-nums tracking-tighter">{testimonials.length}</p>
            ...
            <div
              ...
              <div className="relative z-10 flex flex-col h-full justify-between">
                ...
                <div>
                  <p className="text-xl md:text-3xl font-black mb-0.5 tabular-nums tracking-tighter">{testimonials.filter(t => t.status === 'active').length}</p>
            ...
            <div
              ...
              className={`group relative p-3 md:p-4 rounded-2xl text-white shadow-md overflow-hidden cursor-pointer transition-all duration-500 hover:scale-[1.02] hover:-translate-y-1 ${statusFilter === 'inactive' ? 'z-20 scale-[1.02] shadow-xl border-2 border-white bg-amber-600' : 'z-10 bg-amber-500'}`}>
              <div className="absolute -right-3 -bottom-3 opacity-15 group-hover:scale-110 transition-transform duration-700">
              ...
              <div className="relative z-10 flex flex-col h-full justify-between">
                ...
                <div>
                  <p className="text-xl md:text-3xl font-black mb-0.5 tabular-nums tracking-tighter">{testimonials.filter(t => t.status === 'inactive').length}</p>
                  <p className="text-[10px] md:text-[15px] font-black uppercase tracking-tight opacity-90">Tạm ẩn</p>
                </div>
              </div>
            </div>
          </>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-4 mb-6 shadow-sm flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] font-black text-gray-600 uppercase tracking-[0.2em] pr-2">Hiển thị {filteredData.length} nội dung</span>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-[600px]">
        <DataTable
          ...
          searchable={false}
          pageSize={25}
          emptyStateIcon={<FileText size={48} className="text-gray-600" />}
      ...
      <Dialog
        ...
        title={
          <div className="flex items-center gap-3">
              ...
              <div>
                  <span className="font-bold text-red-600 block uppercase tracking-tight">Xác nhận xóa nội dung</span>
              </div>
          </div>
        }
        footer={
      ...
      >
        <div className="p-4 bg-gray-50 rounded-2xl border border-gray-300">
          <p className="text-gray-700 text-sm leading-relaxed">Bạn có chắc muốn xóa đánh giá này khỏi trang chủ? Hành động này không thể hoàn tác.</p>
        </div>
      </Dialog>
