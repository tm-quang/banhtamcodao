  ...
  return (
    <AnimateOnScroll>
        <div className="text-center mb-4">
            ...
            <h2 className="mt-3 text-4xl sm:text-5xl font-lobster text-secondary drop-shadow-sm">
        ...
        <div className="flex flex-wrap justify-center items-center gap-3 mb-6 pt-1">
            {/* Các nút danh mục */}
            {categoryNames.map((category) => (
            <button
                key={category}
                onClick={() => setActiveCategory(category)}
                className={`group relative flex flex-col items-center justify-center w-28 h-28 p-4 rounded-2xl border transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg ${
            ...
            >
                ...
                <span className={`mt-2 text-xs font-semibold tracking-wide ${
                    activeCategory === category ? 'text-primary' : 'text-gray-600 group-hover:text-primary'
                }`}>
                    {category}
                </span>
            </button>
            ...
            <Link
                href="/menu"
                className="flex items-center justify-center h-11 px-5 rounded-xl font-semibold bg-primary text-light hover:bg-orange-600 transition-all duration-300 shadow-md hover:-translate-y-1"
            >
                Xem tất cả
            </Link>
        </div>
        ...
        {hasMoreProducts && (
            <div className="text-center mt-10">
                <Link
                    href="/menu"
                    className="bg-secondary text-light font-bold py-3 px-8 rounded-xl text-lg hover:bg-gray-700 transition-colors shadow-lg"
                >
                    Xem thêm sản phẩm
                </Link>
            </div>
